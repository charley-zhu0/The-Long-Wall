...
echo "==> [3/6] 准备目录结构 ..."
# 最终目录结构（与 server/dist/index.js 里的路径计算一致）:
#   the-long-wall-deploy/
#     dist/          ← server 编译产物 (__dirname)
#     maps/          ← section-1.json  (__dirname/../../maps → 这里)
#     client/dist/   ← 静态文件        (__dirname/../../client/dist → 这里)
#     node_modules/  ← server 生产依赖
#     package.json
#     docker-compose.yml
#     nginx-ssl.conf
#     deploy.sh
mkdir -p "$PKG_DIR/dist/rooms"
mkdir -p "$PKG_DIR/client/dist"
mkdir -p "$PKG_DIR/maps"
mkdir -p "$PKG_DIR/certs"

# server 编译产物
cp -r "$REPO_DIR/server/dist/." "$PKG_DIR/dist/"

# server 地图数据
cp -r "$REPO_DIR/server/src/maps/." "$PKG_DIR/maps/"
...
cp "$REPO_DIR/deploy/docker-compose.yml" "$PKG_DIR/"
